...
from poshc2.server.Config import HTTPResponses, PoshInstallDirectory, PayloadsDirectory, BeaconDataDirectory
...
def load_module(module_name):
    file = codecs.open(("%sModules/%s" % (PoshInstallDirectory, module_name)), 'r', encoding='utf-8-sig')
    return file.read()


def load_module_sharp(module_name):
    file = open(("%sModules/%s" % (PoshInstallDirectory, module_name)), 'r+b')
