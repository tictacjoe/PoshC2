...
from poshc2.server.Config import QuickCommand, KillDate, DefaultSleep, DomainFrontHeader, PayloadCommsPort, urlConfig, BindIP, BindPort
                ...
                if RandomURI in self.path and cookieVal:
                    ...
                    if "loadmodule" in executedCmd:
                        ...
                        try:
                            ...
                            if (chunkNumber == "00001") and os.path.isfile('%sdownloads/%s' % (PoshProjectDirectory, filename)):
                                counter = 1
                                while(os.path.isfile('%sdownloads/%s' % (PoshProjectDirectory, filename))):
                                    if '.' in filename:
                            ...
                            if (chunkNumber != "00001"):
                                counter = 1
                                if not os.path.isfile('%sdownloads/%s' % (PoshProjectDirectory, filename)):
                            ...
                            output_file = open('%sdownloads/%s' % (PoshProjectDirectory, filename), 'ab')
                            try:
