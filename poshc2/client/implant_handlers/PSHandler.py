...
from poshc2.server.Config import PayloadsDirectory, PoshInstallDirectory, PoshProjectDirectory, SocksHost
...
def handle_ps_command(command, user, randomuri, implant_id):
    ...
    if command.startswith("beacon") or command.startswith("set-beacon") or command.startswith("setbeacon"):
        ...
        do_back(user, command, randomuri)
        return
        ...
        do_quit(user, command, randomuri)
        return
...
def do_clear(user, command, randomuri):
    return do_back(user, command, randomuri)
...
def do_install_servicelevel_persistencewithproxy(user, command, randomuri):
    C2 = get_c2server_all()
    if C2[11] == "":
        print(Colours.RED, "Need to run createproxypayload first", Colours.GREEN)
        return
    else:
...
def do_invoke_psexecdaisypayload(user, command, randomuri):
    ...
    if os.path.isfile(("%s%spayload.bat" % (PayloadsDirectory, daisyname))):
        ...
        print(Colours.RED, "Need to run createdaisypayload first", Colours.GREEN)
        return
...
def do_invoke_runas(user, command, randomuri):
    ...
    if "-credid" in params:
        ...
        if creds['Password']:
            ...
            print(Colours.RED, "invoke-runas does not support hash authentication", Colours.GREEN)
            return
