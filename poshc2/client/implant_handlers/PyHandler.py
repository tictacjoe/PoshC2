...
from poshc2.server.DB import new_task, update_sleep, unhide_implant, kill_implant, get_implantdetails, get_pid, new_c2_message
...
def handle_py_command(command, user, randomuri, startup, implant_id, commandloop):
    ...
    if command.startswith("beacon") or command.startswith("set-beacon") or command.startswith("setbeacon"):
        ...
        modules = os.listdir("%s/Modules/" % PoshInstallDirectory).sort()
        print("")
        print("[+] Available modules:")
        print("")
        for mod in modules:
